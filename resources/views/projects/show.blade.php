...
<head>
    ...
    <title>{{ $project->title }} - Artesanos Junín</title>
...
<body class="bg-gradient-to-br from-purple-50 to-indigo-100 min-h-screen py-12 px-4">
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <!-- Botón Volver -->
        <div class="mb-8">
            <a href="{{ url()->previous() }}" class="inline-flex items-center bg-indigo-600 text-white font-semibold px-5 py-2.5 rounded-lg shadow-md hover:bg-indigo-700 transition">
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-purple-700 p-8 text-white">
                ...
                <div class="flex flex-wrap items-center gap-4 mt-4 text-sm opacity-90">
                    <span><i class="fas fa-calendar mr-1"></i> Publicado el {{ $project->created_at->format('d/m/Y') }}</span>
            ...
            <div class="p-8">
                ...
                <div class="grid md:grid-cols-3 gap-8 mb-10">
                    ...
                    <div class="text-center md:text-left">
                        @if($project->client?->user)
                            <div class="w-24 h-24 mx-auto md:mx-0 bg-indigo-100 rounded-full flex items-center justify-center text-4xl font-bold text-indigo-600 mb-3">
                                {{ strtoupper(substr($project->client->user->name, 0, 2)) }}
                            </div>
                            <p class="text-sm text-gray-600">Publicado por</p>
                            <p class="text-xl font-bold text-gray-800">{{ $project->client->user->name }}</p>
                    ...
                    <div class="text-center md:text-right">
                        ...
                        @if($project->price)
                            <p class="text-4xl font-bold text-indigo-600">S/ {{ number_format($project->price, 2) }}</p>
                ...
                <div class="mt-10 flex flex-wrap gap-4 justify-center md:justify-end">
                    @auth
                        @if(auth()->user()->isClient() && $project->client?->id == auth()->user()->client?->id && $project->status === 'pending')
                            <a href="{{ route('projects.edit', $project) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3 px-6 rounded-lg shadow transition">
                                <i class="fas fa-edit mr-2"></i> Editar Proyecto
                            </a>
                        @endif

                        @if(auth()->user()->isArtisan() && $project->artisan?->id == auth()->user()->artisan?->id)
                            <a href="{{ route('projects.edit', $project) }}" class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg shadow transition">
                                <i class="fas fa-tools mr-2"></i> Actualizar Progreso
                            </a>
                        @endif

                        @if($project->artisan?->user && auth()->user()->isClient())
        ...
        @auth
            @if($project->status === 'completed' && auth()->user()->isClient() && $project->client?->id == auth()->user()->client?->id)
                ...
                @if($project->rating)
                    <div class="mt-10 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-2xl p-8 text-center">
                        ...
                        <div class="flex justify-center gap-3 text-5xl">
                            @for($i = 1; $i <= 5; $i++)
                                <i class="fas fa-star {{ $i <= $project->rating ? 'text-yellow-500' : 'text-gray-300' }}"></i>
                            @endfor
                        </div>
                        @if($project->rating_comment)
                            <p class="mt-5 text-lg text-gray-700 italic max-w-2xl mx-auto">"{{ $project->rating_comment }}"</p>
                        @endif
                ...
                @else
                    <div class="mt-10 bg-white border-2 border-indigo-200 rounded-2xl p-10 shadow-xl">
                        ...
                        <form action="{{ route('projects.rate', $project) }}" method="POST" class="max-w-2xl mx-auto space-y-8">
                            @csrf
